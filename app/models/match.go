...
type Match struct {
	...
	MatchDate    time.Time `gorm:"column:match_date;not null" json:"match_date"`
...
// IsEditable returns true if the match date is within 24 hours from now (or in the past).
func (m *Match) IsEditable() bool {
	return time.Until(m.MatchDate) <= 24*time.Hour
}
