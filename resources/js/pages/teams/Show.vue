      ...
      <v-col cols="12" md="8">
        <v-card>
          <v-card-title class="d-flex justify-space-between align-center">
            ...
            <v-btn size="small" color="primary" variant="tonal" prepend-icon="mdi-plus" @click="openAddPlayerDialog">
              Agregar Jugador
            </v-btn>
          </v-card-title>
          <v-table v-if="players.length > 0">
            ...
            <tbody>
              <tr v-for="tp in players" :key="tp.player?.id || tp.id">
                ...
                <td>
                  <v-btn icon="mdi-close" size="x-small" variant="text" color="error" @click="removePlayer(tp.player?.id || tp.player_id)" />
                </td>
              </tr>
            </tbody>
          </v-table>
        ...
        <v-card>
          ...
          <v-card-text>
            <div class="d-flex justify-space-between mb-2">
              ...
              <span class="font-weight-bold">{{ players.length }}</span>
            </div>
          </v-card-text>
        </v-card>
      </v-col>
    ...
    <v-dialog v-model="showAddPlayerDialog" max-width="400">
      <v-card>
        ...
        <v-card-text>
          ...
          />
          <p v-if="availablePlayers.length === 0 && !loadingPlayers" class="text-medium-emphasis">
            No hay jugadores disponibles. Primero crea jugadores.
          </p>
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="showAddPlayerDialog = false">Cancelar</v-btn>
          <v-btn color="primary" :disabled="!selectedPlayerId" @click="addPlayer">Agregar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
