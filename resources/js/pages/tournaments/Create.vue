...
const form = useForm({
  name: "", description: "", format: "round_robin", game_type: "5v5",
  start_date: "", end_date: "", venue_id: null as number | null,
});
    ...
    <v-card>
      <v-card-text class="pa-6">
        <v-form @submit.prevent="submit">
          ...
          <v-row>
            <v-col cols="12" md="6">
              <v-select v-model="form.format" :items="formats" label="Formato" class="mb-4" />
            </v-col>
            <v-col cols="12" md="6">
              <v-select v-model="form.game_type" :items="gameTypes" label="Tipo de Juego" class="mb-4" />
            </v-col>
          </v-row>
          <v-row>
            <v-col cols="12" md="6">
              <v-text-field v-model="form.start_date" label="Fecha Inicio" type="date" class="mb-4" />
            </v-col>
            <v-col cols="12" md="6">
              <v-text-field v-model="form.end_date" label="Fecha Fin" type="date" class="mb-4" />
            </v-col>
          </v-row>
          ...
          <div class="d-flex ga-4">
            ...
            <v-btn variant="outlined" href="/tournaments">Cancelar</v-btn>
          </div>
        </v-form>
      </v-card-text>
    </v-card>
