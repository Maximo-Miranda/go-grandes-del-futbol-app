...
onMounted(() => {
  ...
  if (flash.value?.success) showSnackbar(flash.value.success, "success");
  else if (flash.value?.error) showSnackbar(flash.value.error, "error");
  else if (flash.value?.warning) showSnackbar(flash.value.warning, "warning");
});
...
<template>
  ...
  <v-app>
    <v-app-bar color="primary" density="comfortable" flat>
      <v-app-bar-nav-icon v-if="isMobile" @click="mobileDrawer = !mobileDrawer" />
      <v-app-bar-nav-icon v-else @click="toggleRail" />
      <v-app-bar-title>
        <Link href="/dashboard" class="text-white text-decoration-none d-flex align-center">
      ...
      <v-menu offset-y>
        <template #activator="{ props }">
          <v-btn v-bind="props" variant="text" class="text-white">
            <v-avatar size="32" color="primary-darken-1" class="mr-2">
              <span class="text-body-2">{{ authUser?.name?.charAt(0) || "U" }}</span>
            </v-avatar>
            <span class="d-none d-sm-inline">{{ authUser?.name || "Usuario" }}</span>
    ...
    <v-navigation-drawer v-if="!isMobile" v-model="drawer" :rail="rail" permanent>
      <v-list density="compact" nav>
        <v-list-item
          v-for="item in navItems" :key="item.to"
          :prepend-icon="item.icon" :title="item.title" :href="item.to"
          rounded="lg" color="primary"
        />
      </v-list>
      <template #append>
        <v-list density="compact" nav>
          <v-list-item prepend-icon="mdi-logout" title="Cerrar Sesión" @click="logout" rounded="lg" />
        </v-list>
      </template>
    </v-navigation-drawer>
    ...
    <v-navigation-drawer v-if="isMobile" v-model="mobileDrawer" temporary>
      <v-list density="compact" nav>
        <v-list-item
          v-for="item in navItems" :key="item.to"
          :prepend-icon="item.icon" :title="item.title" :href="item.to"
          rounded="lg" color="primary" @click="mobileDrawer = false"
        />
      </v-list>
      <template #append>
        <v-list density="compact" nav>
